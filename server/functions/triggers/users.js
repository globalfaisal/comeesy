...
const updateUserImageUrlInAllReferences = async snapshot => {
...
const updateNameOfUserInAllReferences = async snapshot => {
  if (snapshot.before.data().name !== snapshot.after.data().name) {
    try {
      ...
      await db
        ...
        .then(data => {
          if (!data.empty) {
            ...
            console.log(
              'Changed name reference at all docs in likes collection'
            );
            return batch.commit();
          }
          return null;
...
exports.onUserAvatarChange = snapshot => {
  updateUserImageUrlInAllReferences(snapshot);
};

exports.onNameChange = spanshot => {
  updateNameOfUserInAllReferences(spanshot);
};
